...
// Function prototypes
void cleanup();
void readFiles(float *X, float *Y, float *Z, char *x, char *y, char *z, int n);
void writeFiles(int *C, float *CX, float *CY, float *CZ, int k, int n);
...
int main(int argc, char **argv) {
	...
	if (n == 100) {
		...
		zFilename = "../Data/Z100.txt";
	}
	else if (n == 1000) {
		xFilename = "../Data/X1000.txt";
		yFilename = "../Data/Y1000.txt";
		zFilename = "../Data/Z1000.txt";
	}
	else if (n == 2000) {
		xFilename = "../Data/X2000.txt";
		yFilename = "../Data/Y2000.txt";
		zFilename = "../Data/Z2000.txt";
	}
	else if (n == 10000) {
		xFilename = "../Data/X10000.txt";
		yFilename = "../Data/Y10000.txt";
		zFilename = "../Data/Z10000.txt";
	}
	else if (n == 100000) {
		xFilename = "../Data/X100000.txt";
		yFilename = "../Data/Y100000.txt";
		zFilename = "../Data/Z100000.txt";
	}
	else if (n == 1000000) {
		xFilename = "../Data/X1000000.txt";
		yFilename = "../Data/Y1000000.txt";
		zFilename = "../Data/Z1000000.txt";
	}
	else {
	...
	for (int i = 0; i < k; i++)
	{
		...
		CZ[i] = range * ((float)rand() / (float)RAND_MAX);
	}

	readFiles(X, Y, Z, xFilename, yFilename, zFilename, n);
	...
	platformID = findPlatform("Intel(R) FPGA SDK for OpenCL(TM)");
        if (platformID == NULL) {
                platformID == findPlatform("Intel(R) FPGA SDK for OpenCL");
                if(platformID == NULL){
                        platformID == findPlatform("Intel(R) FPGA SDK");
                        if(platformID == NULL){
                                platformID = findPlatform("Intel(R) FPGA");
                                if(platformID == NULL)
                                {
	...
	cl_mem zMem = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), Z, &ret);
	...
	cl_mem czMem = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, k * sizeof(float), CZ, &ret);
	...
	cl_mem ozMem = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, k * sizeof(float), oldZ, &ret);
	...
	cl_mem szMem = clCreateBuffer(context, CL_MEM_READ_WRITE, numGroups * k * sizeof(int), NULL, &ret);
	...
	if (!xMem || !yMem || !zMem || !cxMem || !cyMem || !czMem || !gcMem || !sxMem || !syMem ||  !szMem || !oxMem || !oyMem || !ozMem || !fMem || !clMem)
	{
	...
	clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&zMem);
	clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *)&cxMem);
	clSetKernelArg(kernel, 5, sizeof(cl_mem), (void *)&cyMem);
	clSetKernelArg(kernel, 6, sizeof(cl_mem), (void *)&czMem);
	clSetKernelArg(kernel, 7, sizeof(cl_mem), (void *)&sxMem);
	clSetKernelArg(kernel, 8, sizeof(cl_mem), (void *)&syMem);
	clSetKernelArg(kernel, 9, sizeof(cl_mem), (void *)&szMem);
	clSetKernelArg(kernel, 10, sizeof(cl_mem), (void *)&gcMem);
	clSetKernelArg(kernel, 11, GROUPSIZE * sizeof(int), NULL);
	clSetKernelArg(kernel, 12, GROUPSIZE * sizeof(float), NULL);
	clSetKernelArg(kernel, 13, GROUPSIZE * sizeof(float), NULL);
	clSetKernelArg(kernel, 14, sizeof(int), (void *)&n);
	clSetKernelArg(kernel, 15, sizeof(int), (void *)&k);
	...
	rets[2] = clSetKernelArg(kernel2, 2, sizeof(cl_mem), (void *)&czMem);
	rets[3] = clSetKernelArg(kernel2, 3, sizeof(cl_mem), (void *)&sxMem);
	rets[4] = clSetKernelArg(kernel2, 4, sizeof(cl_mem), (void *)&syMem);
	rets[5] = clSetKernelArg(kernel2, 5, sizeof(cl_mem), (void *)&szMem);
	rets[6] = clSetKernelArg(kernel2, 6, sizeof(cl_mem), (void *)&oxMem);
	rets[7] = clSetKernelArg(kernel2, 7, sizeof(cl_mem), (void *)&oyMem);
	rets[8] = clSetKernelArg(kernel2, 8, sizeof(cl_mem), (void *)&ozMem);
	rets[9] = clSetKernelArg(kernel2, 9, sizeof(cl_mem), (void *)&gcMem);
	rets[10] = clSetKernelArg(kernel2, 10, GROUPSIZE * sizeof(int), NULL);
	rets[11] = clSetKernelArg(kernel2, 11, GROUPSIZE * sizeof(float), NULL);
	rets[12] = clSetKernelArg(kernel2, 12, GROUPSIZE * sizeof(float), NULL);
	rets[13] = clSetKernelArg(kernel2, 13, k * sizeof(int), NULL);
	rets[14] = clSetKernelArg(kernel2, 14, sizeof(int), (void *)&k);
	rets[15] = clSetKernelArg(kernel2, 15, sizeof(int), (void *)&numGroups);
	rets[16] = clSetKernelArg(kernel2, 16, sizeof(cl_mem), (void *)&fMem);
	for (int i = 0; i < 14; i++)
	{
	...
	// Reading final results which are clusters point of X, Y and Z axis
	ret = clEnqueueReadBuffer(queue, czMem, CL_TRUE, 0, k * sizeof(float), CZ, 0, NULL, NULL);
	...
	writeFiles(clusters, CX, CY, CZ, k, n);
...
void writeFiles(int *C, float *CX, float *CY, float *CZ, int k, int n)
{
	...
	FILE *czFile = fopen("../OCLCZ.txt", "w");
	for (int i = 0; i < k; i++)
	{
		fprintf(czFile, "%f", CZ[i]);
		if (i < k - 1)
		{
			fprintf(czFile, "\n");
		}
