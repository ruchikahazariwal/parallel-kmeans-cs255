...
float dist(float px, float py, float pz, float cx, float cy, float cz)
{
	float distance = (fabs(px - cx)) + (fabs(py - cy)) + (fabs(pz - cz));
	return distance;
}
...
)
{
	...
	for (int i = 0; i < k; i++)
	{
		distance = dist(x, y, z, sharedX[i], sharedY[i], sharedZ[i]);
	...
	for (int i = 0; i < k; i++)
	{
		...
		while (!stop)
		{
			...
			if (lid < thresh)
			{
				...
				sharedZ[lid] += sharedZ[lid + stride];
	...
	int k, int numGroups, __global int *flag)
{
	...
	if (numGroups  > 1)
	{
		while (!stop)
		{
			...
			if (lid < (thresh * k))
			{
				...
				sharedZ[lid] += sharedZ[lid + (stride * k)];
	...
	if (lid < k)
	{
		...
		if (count != 0)
		{
			...
			cZ[lid] = sharedZ[lid] / count;
		}
		float diffx = fabs(cX[lid] - oldx[lid]);
		float diffy = fabs(cY[lid] - oldy[lid]);
		float diffz = fabs(cZ[lid] - oldz[lid]);
		flags[lid] = (diffx == 0 && diffy == 0 && diffz == 0) ? 1 : 0;
		oldx[lid] = cX[lid];
		oldy[lid] = cY[lid];
		oldy[lid] = cZ[lid];
		counts[lid] = 0;
